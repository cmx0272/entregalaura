...
//	Préstamos gestionados por un empleado
	...
	public Set<Prestamo> prestamosDeEmpleado(String dni) {
		Set<Prestamo> pr = this.prestamos().todos()
				.stream()
...
// Empleado más joven
	public Optional<Persona> empleadoMasJoven() {
		return this.empleados().todos().stream().map(Empleado::persona)
				.max((p1,p2) -> p1.fechaDeNacimiento().compareTo(p2.fechaDeNacimiento()));
	}
	
// Número de cuentas de cada cliente
	public Map<String, Integer> numeroDeCuentasDeCliente() {
		return this.cuentas().todas().stream()
				.collect(Collectors.groupingBy(Cuenta::getDni, Collectors.counting()))
				.entrySet().stream()
